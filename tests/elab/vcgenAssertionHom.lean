...
exception postcondition type. `Thrown` is such a type outside the `EPost` hierarchy: the
verification condition for a concrete exception postcondition is stated for the factor of the
assertion tuple that `Thrown` converts to, and a schematic factor is assigned instead of yielding a
...
instance : AssertionHom Thrown ((String → Prop) × Unit) (fun t => (t.onThrow, ())) where
  le_of_hom_le h := h.1
  hom_bot := by
    have h : (⊥ : Thrown).onThrow = ⊥ :=
      PartialOrder.rel_antisymm (bot_le (⟨⊥⟩ : Thrown)) (bot_le _)
    rw [h, Subsingleton.elim () (⊥ : Unit), Prod.mk_bot]
...
/-- The schematic assertion of `boom'_spec` is assigned the assertion of the goal, so the only
verification condition is the one for the precondition. -/
