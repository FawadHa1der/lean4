...
def setupImports
  ...
  -- cache (the registry of patch 0032) — so a header change re-runs setup
  ...
  where
    ...
    mkLspOutputChannel maxDocVersion : IO (Std.Channel OutputMessage) := do
      ...
      let _ ← chanOut.forAsync (prio := .dedicated) fun ⟨msg?, serialized⟩ => do
        ...
        -- `writeSerializedLspMessage` flushes after every frame; the wasm host
        -- stream (Emscripten TTY) is line-buffered and frame bodies carry no
        -- trailing newline, so that flush is load-bearing there.
