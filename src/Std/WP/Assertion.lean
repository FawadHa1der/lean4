...
into an assertion `T` that exposes the layers as a tuple, and `le_of_hom_le` reflects an entailment
between the tuples back to an entailment between the originals. For a program in
`ExceptT Nat (ExceptT String (StateM σ))`, `T` is
`(Nat → σ → Prop) × (String → σ → Prop) × Unit`, and each factor is the assertion of one layer.
-/
