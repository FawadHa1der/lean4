...
- **Exports are generated by category, the committed `wanted` list is gone.**
  `emscripten-exports.wanted.txt` was exactly {every `___boxed` wrapper} ∪
  {every module initializer} ∪ {every constant cell} of the 4.33 tree, and
  "seed ∪ (wanted ∩ defined)" could only shrink: v4.34.0 would have linked
...
- **23 seed names removed**: upstream dropped their `@[export lean_*]`
  (`lean_add_alias`, `lean_is_class`, `lean_mk_empty_environment`, …); no C++
  or JS caller remained. They are reachable through their boxed wrappers like
